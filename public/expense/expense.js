...
function expense(e) {
    ...
    axios.post('http://3.110.117.219:3000/expense/add-expense', expenseDetails, { headers: { Authorization: token } })
        .then(result => {
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    axios.get('http://3.110.117.219:3000/expense/get-expense', { headers: { "Authorization": token } })
        .then(result => {
...
function deleteExpenseFromUi(e, obj) {
    ...
    axios.delete(`http://3.110.117.219:3000/expense/delete-expense/${obj}`, { headers: { Authorization: token } });
...
function rzp(e) {
    ...
    axios.get('http://3.110.117.219:3000/premium/buy-premium', { headers: { Authorization: token } })
        .then(result => {
            const options = {
                key_id: result.data.key_id,
                order_id: result.data.order.id,
                handler: async function (result) {
                    const res = await axios.post('http://3.110.117.219:3000/premium/updatetransictionstatus', {
...
async function leadeboard(event){
   ...
   const leaderboarddata = await  axios.get('http://3.110.117.219:3000/premium/showleaderboard',{headers:{Authorization:token}});
...
function download(){
    const token = localStorage.getItem('token');
    axios.get('http://3.110.117.219:3000/expense/download',{headers:{Authorization:token}})
    .then(response=>{
